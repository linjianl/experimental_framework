#' GetExpGroupTrivago
#' Gets history of experiments and groups for Trivago with salt
...
#' @export
#' 
GetExpGroupTrivago <- function( # need to change to the start date of latest experiments 
...
getPerfExpTrivago <- function(baseDate = format(Sys.Date()-30, "%F")
...
#' GetExpGroupTripadvisor
#' Gets history of experiments and groups for Tripadvisor with salt
#' make it consistent across accounts
#' 
#' @param 
#' baseDate: date string in the format "YYYY-MM-DD"
...
GetExpGroupTripadvisor <- function(endDate = format(Sys.Date(), "%F")){
    require(data.table, quietly = T)
    confFile <- "/shared/ppc/ppc_conf/sp/Tripadvisor/config/temp.conf" # temporary test version
    txtConf  <- scan(file = confFile,what = "char",quiet = T,comment.char = "#")
    ## peruse file sequentially for each set of experiments and keep data.table with summary
    expGroups = data.table()
    numExp   <- grep("<experiments",txtConf)
...
#' getPerfExpTripadvisor
#' Gets performance for tripadvisor experiments, adaptations of get_perf_hotel_exp in tripAdvisorFuncs.R
#' 
#' @param 
#' startDate, endDate
#' 
#' @return data.table with results
#' 
#' @author Lin Jia, Andreas Pagnous 
#' 
#' @import data.table
#' 
#' @export
...
getPerfExpTripadvisor <- function(startDate = format(Sys.Date()-7, "%F")
                                  ,endDate = format(Sys.Date(), "%F")
                                  ,lsDistribution = NULL
    ...
    ## clicks and cost from ppc_trip.Trip_API_cost_granular --> TripAPICostSiloHotel_YYYYMM
    ads <- data.table()
    for (ym in unique(format(seq(as.Date(startDate),as.Date(endDate),1),"%Y%m"))) {
        q1 = sprintf("select date
                     ...
                     ,sum(cost_EUR) cost
                     ,count(*) clicks
                     from ppc_trip.TripAPIClickCost_CET_%1$s a
    ...
    if (nrow(ads) > 0 & nrow(sale) > 0) {
        ...
        exp_overview <- getDataTableMS(sprintf("select experiment_id, salt, logic, weight, grp_list,
                                           participating_markets, placement from ppc_trip.experiments_evaluation_overview
...
#' GetExpGroupGHA
#' Gets history of experiments and groups for GHA with salt
#' make it consistent with TripAdvisor and Trivago
#' 
#' @param 
#' baseDate: date string in the format "YYYY-MM-DD"
#' 
...
#' @export
#' 
GetExpGroupGHA <- function(startDate = format(Sys.Date()-30, "%F")
                           ,endDate = format(Sys.Date(), "%F")){
    require(data.table, quietly = T)
    lsDates  <- format(seq(as.Date(startDate),as.Date(endDate),1),"%F")
    confFile <- "/shared/ppc/ppc_conf/sp/gha/experiment_config/main.conf"
    txtConf  <- scan(file = confFile,what = "char",quiet = T,comment.char = "#")
    ## peruse file sequentially for each set of experiments and keep data.table with summary
    expGroups = data.table()
    numExp   <- grep("<experiments",txtConf)
...
#' 
#' 
getPerfExpGHA <- function(baseDate = format(Sys.Date()-30, "%F")
                          ,endDate = format(Sys.Date(), "%F")
                          ,book_window_num = F
                          ,los_cap = 30
                          #,bw_cap = 360
    ...
    # get bookings and commissions from SPReservation and clicks and cost from GHAStats
    ...
    exp_summ <- getDataTableMS(sprintf("select experiment_id, salt, logic, weight, grp_list detail_group, participating_markets
                                       from ppc_gha.experiments_evaluation_overview 
                                       where experiment_id = '%s' and salt = '%s'", experiment_id, salt))
            ...
            tryCatch({
                ddClicks <- rbind(ddClicks
                                  , getDataTableMS(sprintf("SELECT '%1$s' yyyy_mm_dd, campaign_id
                                                           ...
                                                           FROM ppc_gha.GHA_CampaignStats_%2$s
                                                           WHERE clicks > 0
...
get_campaign_cc <- function() {
    ...
    cc <- getDataTableMS("SELECT upper(country_code) cc, sales_region region from ppc_gha.country_region")
    ...
    cc_campaign <- merge(cc_campaign, getDataTableMS("select id as campaign_id, name as campaign_name
                                                     from mrktctl.GHABid_Campaign"), by = "campaign_id")
    return(cc_campaign)
}
