# /opt/spark/current20/bin/spark-submit generate_smart_salt_table_v2.py --account trivago --run_date 2019-02-01 --ntrials 10000 --group_size 5percent 2> /dev/null
...
def main():
    ...
    salt_table = experiment.get_smart_salt_table_v2().cache()
    salt_table.registerTempTable("salt_table")

    table_name = ('_'.join([args.account, 'smart_salt_table_v2', datetime.strptime(args.run_date, "%Y-%m-%d").strftime("%Y%m%d"),
