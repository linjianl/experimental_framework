# /opt/spark/current20/bin/spark-submit generate_bootstrapped_smart_salt_result_v2.py --account trivago --exp_groups 2 2 2 2 2 2 2 2 2 2 --group_size 10percent --run_date 2019-02-28 --number_of_salts 100 --source_table_name trivago_smart_salt_table_20190201_ntrials_10000_group_size_10percent 2>/dev/null
...
def main():
    ...
    parser.add_argument("--group_size", type=str, default = '25percent')
    ...
    table_name = ('_'.join([args.account, 'smart_salt_bootstrapping_v2',
                            datetime.strptime(args.run_date, "%Y-%m-%d").strftime("%Y%m%d"), 
                            args.group_size,'salt_numbers', str(args.number_of_salts), 'period', str(args.period)]))
