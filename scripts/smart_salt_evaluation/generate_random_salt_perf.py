# /opt/spark/current20/bin/spark-submit generate_random_salt_perf.py  --exp_groups 2 2 2 2 2 2 2 2 2 2  --account trivago --run_date 2019-03-28 --ntrials 10000 2> /dev/null
...
def main():
    ...
    ## random salt perf table
    salt_perf = experiment.get_random_salt_performance().cache()
    repartition_to_blocksize(salt_perf).registerTempTable("salt_perf")
    
    table_name = ('_'.join([args.account, 'random_salt_perf', datetime.strptime(args.run_date, "%Y-%m-%d").strftime("%Y%m%d"),
    ...
    logging.info("Number of rows in the {0} salt table of {1} salts are : {2}".format(args.account, args.ntrials,
                                                                                               salt_perf.count()))
